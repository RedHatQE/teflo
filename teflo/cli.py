...
from .constants import ITERATE_METHOD_CHOICES
from .constants import TASK_LOGLEVEL_CHOICES
...
def print_header():
    ...
    click.echo("Copyright (C) 2022, Red Hat, Inc.")
    click.echo("-" * 50)


@click.group()
@click.option("-v", "--verbose", count=True, help="Add verbosity to the commands.")
...
def show(ctx, scenario, list_labels, vars_data, show_graph, iterate_method):
    ...
    scenario_graph: ScenarioGraph = validate_cli_scenario_option(
        ctx, scenario, cbn.config, vars_data
    )
...
@click.option("-w", "--workspace", default=None, metavar="", help="Scenario workspace.")
@click.option(
    "--log-level",
...
def init(ctx, dirname):
    ...
    if os.path.exists(dirname):
        if not click.confirm(
            "{0} already exists,overwrite it?".format(dirname), default=False
        ):
            ctx.exit()
        else:
    ...
    if alias:
        ...
        if alias in alias_list[0]:
            ...
            if get_task == "run":
                ...
                raise TefloError(
                    f'Error: Task - "{get_task}" is not valid task please use teflo --help'
                )
        else:
