...
from ..helpers import fetch_assets
from ..helpers import fetch_executes
from ..helpers import filter_actions_on_failed_status
from ..helpers import filter_notifications_on_trigger
from ..helpers import filter_notifications_to_skip
from ..helpers import filter_resources_labels
from ..helpers import get_core_tasks_classes
from ..helpers import set_task_class_concurrency
...
class PipelineBuilder(object):
    ...
    def __init__(self, name):
        ...
        # list of tasks
        self.pipeline_template = namedtuple("Pipeline", ("name", "type", "tasks"))
    ...
    def build(
        self, scenario: Scenario, teflo_options, scenario_graph: ScenarioGraph = None
    ):
        ...
        # pipeline init
        pipeline = self.pipeline_template(self.name, self.task_cls_lookup(), list())
            ...
            for task in scenario_get_tasks:
                if task["task"].__task_name__ == self.name:
                    pipeline.tasks.append(
                        set_task_class_concurrency(task, task["resource"])
                    )
        ...
        if self.name.lower() in ["validate", "orchestrate", "cleanup"]:
            ...
            if self.name != "cleanup":
                scenario_actions = filter_actions_on_failed_status(
                    scenario.get_actions()
                )
            else:
            ...
            for action in filter_resources_labels(scenario_actions, teflo_options):
                for task in action.get_tasks():
                    if task["task"].__task_name__ == self.name:
                        ...
                        assets = filter_resources_labels(
                            scenario_graph.get_assets(), teflo_options
                        )
        ...
        if self.name.lower() in ["validate", "execute"]:
            ...
            for execute in filter_resources_labels(
                scenario.get_executes(), teflo_options
            ):
        ...
        if self.name.lower() in ["validate", "report"]:
            ...
            for report in filter_resources_labels(
                scenario.get_reports(), teflo_options
            ):
                for task in report.get_tasks():
                    if task["task"].__task_name__ == self.name:
                        ...
                        executes = filter_resources_labels(
                            scenario_graph.get_executes(), teflo_options
                        )
        ...
        if self.name.lower() in ["validate"]:
            ...
            for notification in filter_notifications_to_skip(
                scenario.get_notifications(), teflo_options
            ):
...
class NotificationPipelineBuilder(PipelineBuilder):
    ...
    def build(
        ...
        if (
            getattr(scenario, "passed_tasks", None) is not None
            and getattr(scenario, "failed_tasks", None) is not None
        ):

            scenario_notifications.extend(
                [
                    item
                    for item in filter_notifications_to_skip(
                        scenario.get_notifications(), teflo_options
                    )
                ]
            )
            scenario_notifications = [
                item
                for item in filter_notifications_on_trigger(
                    self.trigger,
                    scenario_notifications,
