...
class ResourceChecker(object):
    ...
    def validate_resources(self):
        ...
        if (
            getattr(self.scenario, "resource_check").get("monitored_services", None)
            and self.config["RESOURCE_CHECK_ENDPOINT"]
        ):
            ...
            if not self.config["RESOURCE_CHECK_ENDPOINT"].__contains__(
                "https://internal.status.redhat.com/"
            ):
                self.__check_service_statuspage()
            else:
                self.__check_service()
        if getattr(self.scenario, "resource_check").get("playbook", None) or getattr(
            self.scenario, "resource_check"
        ).get("script", None):
    ...
    def __check_custom_resource(self):
        ...
        self.ans_service = AnsibleService(
            self.config, hosts=["localhost"], all_hosts=[], ansible_options=None
        )
    ...
    def __check_service_statuspage(self):
        ...
        for comp in component_names:
            ...
            statuspage = StatusPageHelper(
                proxyserver_url=self.config["RESOURCE_CHECK_ENDPOINT"]
            )
            for attempts in range(1, 6):
                ...
                if service_entry is None:
                    LOG.info(
                        "{:>40} {:<9} - Can't Find Service{}".format(
                    ...
                    LOG.info(
                        "{:>40} {:<9} - Attempts {}".format(
                            comp.upper(),
                            ": UP"
                            if service_entry.get("status") == "operational"
                            else ": DOWN",
                            attempts,
                        )
                    )
