...
from ..constants import DEFAULT_CONFIG_SECTIONS
from ..constants import DEFAULT_TASK_CONCURRENCY
...
class Config(dict):
    ...
    def __set_credentials__(self, **kwargs):
        ...
        for section in getattr(parser, "_sections"):
            ...
            for option in parser.options(section):
                _credentials[option] = parser.get(section, option)
    ...
    def __set_executor__(self):
        ...
        for section in getattr(self.parser, "_sections"):
            ...
            for option in self.parser.options(section):
                self.__setitem__(
                    (executor + "_" + option).upper(), self.parser.get(section, option)
                )
    ...
    def __set_importer__(self):
        ...
        for section in getattr(self.parser, "_sections"):
            ...
            for option in self.parser.options(section):
                self.__setitem__(
                    (importer + "_" + option).upper(), self.parser.get(section, option)
                )
    ...
    def __set_task_concurrency__(self):
        ...
        for section in getattr(self.parser, "_sections"):
            ...
            for option in self.parser.options(section):
                _concurrency_settings.update(
                    {option.upper(): self.parser.get(section, option)}
                )
    ...
    def __get_aliases__(self):
        ...
        for section in getattr(self.parser, "_sections"):
            ...
            for option in self.parser.options(section):
                _aliases_options[option] = self.parser.get(section, option)
    ...
    def load(self):
        ...
        """
        files = ["/etc/teflo/teflo.cfg", os.path.join(os.getcwd(), "teflo.cfg")]
