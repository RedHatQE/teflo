...
from ..helpers import get_default_provisioner_plugin
from ..helpers import get_provisioner_plugin_class
from ..helpers import get_provisioners_plugins_list
from ..helpers import is_provider_mapped_to_provisioner
from ..provisioners import AssetProvisioner
from ..tasks import CleanupTask
from ..tasks import ProvisionTask
...
class Asset(TefloResource):
        ...
        **kwargs
        ...
        if parameters.pop("role", None):
            raise TefloResourceError(
                "Role is not supported as a provisioner parameter. "
        ...
        try:
            ...
            if self._name in self._groups:
                raise TefloResourceError(
                    "Asset name %s cannot be same as groups name %s"
                    % (self._name, self._groups)
                )
        except KeyError:
            self.logger.warning(
                "Groups parameter was not set for asset %s." % self.name
            )
            del self.groups
        ...
        if (
            "ip_address" in parameters
            and "provider" not in parameters
            and "provisioner" not in parameters
        ):
            ...
            if provisioner_name is None and self.has_provider:
                try:
                    ...
                    raise TefloResourceError(
                        "Specified provider is not supported by a provisioner."
                    )
                ...
                if found_name:
                    if self.has_provider and is_provider_mapped_to_provisioner(
                        self._provider, provisioner_name
                    ):
                        self._provisioner = get_provisioner_plugin_class(
                            provisioner_name
                        )
                    else:
                        self._provisioner = get_provisioner_plugin_class(
                    ...
                    self.logger.error(
                        "Provisioner %s for asset %s is invalid."
                        % (provisioner_name, self.name)
                    )
                    raise TefloResourceError(
                        "The specified provisioner is not valid for the asset type."
                    )
            else:
                raise TefloResourceError(
                    "Could not find a provisioner to satisfy the asset type."
                )
    ...
    def metadata(self, value):
        ...
        raise AttributeError(
            "You cannot set metadata directly. " "Use function ~Asset.set_metadata"
        )
    ...
    def ansible_params(self, value):
        ...
        raise AttributeError(
            "You cannot set the ansible parameters directly."
    ...
    def provider(self, value):
        ...
        raise AttributeError(
            "You cannot set the asset provider after asset " "class is instantiated."
        )
    ...
    def provisioner(self, value):
        ...
        raise AttributeError(
            "You cannot set the asset provisioner plugin after asset "
    ...
    def credential(self, value):
        ...
        raise AttributeError(
            "You cannot set the asset credential after asset " "class is instantiated."
        )
    ...
    def profile(self):
        ...
        filtered_attr = {
            ...
            and k not in ["is_static", "has_provider", "no_inventory"]
        }
        ...
        for f in self._fields:
            if f == "groups" and hasattr(self, f):
                ...
                profile.update(
                    {"provisioner": getattr(self.provisioner, "__plugin_name__")}
                )
                if (
                    hasattr(self, "provider")
                    and len(getattr(self, "provider", {})) != 0
                ):
                ...
                if len(getattr(self, "provider", {})) == 0:
                    ...
                    profile.get("provider").update(
                        credential=self.credential.get("name")
                    )
                continue
