...
class ActionOrchestrator(LoggerMixin, TimeMixin):
    ...
    def validate(self):
        ...
        try:
            ...
            self.logger.info(
                "successfully validated scenario Action %s against the schema!"
    ...
    def run(self):
        ...
        if res == 0:
            ...
            self.logger.info(
                "Orchestration passed : Successfully completed orchestrate action: %s."
                % self.plugin.action_name
            )
        else:
            setattr(self.action, "status", 1)
            raise TefloOrchestratorError(
                "Orchestration failed : Failed to perform  %s" % self.plugin.action_name
            )
