...
class AnsibleOrchestratorPlugin(OrchestratorPlugin):
    ...
    def backwards_compat_check(self):
        ...
        if os.sep in self.action_name:
            raise TefloOrchestratorError(
                "Using name field to provide ansible_script/ansible_playbook path is "
        ...
        if isinstance(self.script, bool):
            raise TefloOrchestratorError(
                "ansible_script as boolean is not supported any more. "
    ...
    def validate(self):
        ...
        schema_validator(
            schema_data=self.build_profile(self.action),
        ...
        if self.script:
            if os.path.exists(self.script.get("name").split(" ", 1)[0]):
                self.logger.debug(
                    "Found Action resource script %s" % self.script.get("name")
                )
            elif os.path.exists(
                os.path.join(
                ...
                raise TefloOrchestratorError(
                    "Cannot find Action resource script %s" % self.script.get("name")
                )
        elif self.playbook:
            if os.path.exists(self.playbook.get("name").split(" ", 1)[0]):
                # os.path.exists(os.path.join(self.playbook.get('name').split(' ', 1)[0]):
                self.logger.debug(
                    "Found Action resource playbook %s" % self.playbook.get("name")
                )
            # it doesn't have short_lib_name here, so it can't find the file
                ...
                coll_playbook_path = self.ans_service.get_user_ansiblg_config(
                ...
                if len(coll_playbook_path) > 1:
                    ...
                    if os.path.exists(ws_coll_path):
                        self.logger.debug(
                            "Found Action resource playbook %s" % ws_coll_path
                        )
                elif True:
                    self.ans_service.download_roles()
                    new_path = self.ans_service.get_playbook_path(
                        self.workspace, playbook=self.playbook.get("name")
                    )
                    if os.path.exists(new_path):
                        self.playbook["name"] = new_path
                        self.logger.debug(
                            "Found Action resource playbook %s"
    ...
    def __playbook__(self):
        ...
        if os.path.exists(
            ...
            name_split[0] = os.path.join(self.config["WORKSPACE"], name_split[0])
        ...
        if results[0] != 0:
            if results[1]:
                self.logger.debug(
                    "During the failure these messages were thrown as a part"
                    " of stderr:\n %s " % results[1]
                )
            raise TefloOrchestratorError(
            ...
            self.logger.info(
                "Successfully completed playbook : %s" % self.playbook["name"]
            )
    ...
    def __shell__(self):
        ...
        for shell in self.shell:
            ...
            if result["rc"] != 0:
                ...
                self.logger.info(
                    "Successfully completed command : %s" % shell["command"]
                )
