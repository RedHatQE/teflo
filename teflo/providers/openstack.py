...
class OpenstackProvider(CloudProvider):
    ...
    def __init__(self):
        ...
        self.comm_opt_params = [("keypair", [str])]

        self.teflo_comm_opt_params = [("floating_ip_pool", [str])]

        self.linchpin_comm_opt_params = [("fip_pool", [str])]
        ...
        self.opt_credential_params = [("region", [str]), ("domain_name", [str])]
    ...
    def validate_common_opt_params(self, resource_name, provisioner_name, params):
        ...
        for item in self.linchpin_comm_opt_params:
            ...
            msg = "Resource %s : optional param '%s' " % (resource_name, param)
            err_msg = (
                "Resource %s : optional param '%s' or it's value type is "
                "not support by the openstack-libcloud provisioner"
                % (resource_name, param)
            )
            try:
                param_value = params[param]
        ...
        for item in self.teflo_comm_opt_params:
            ...
            try:
                ...
                if "linchpin" in provisioner_name:
                    self.logger.warning(
                        msg
                        + "is forward compatible with the linchpin provisioner. It will be translated "
    ...
    def validate_opt_linchpin_params(self, resource_name, provisioner_name, params):
        ...
        if "linchpin" in provisioner_name:
            ...
            for item in self.linchpin_only_opt_params:
                param, param_type = item[0], item[1]
                msg = (
                    "Resource %s : optional param '%s' is "
                    "not support by the openstack-libcloud provisioner"
                    % (resource_name, param)
                )
                try:
