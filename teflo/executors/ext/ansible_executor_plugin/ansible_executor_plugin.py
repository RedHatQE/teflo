...
from teflo.helpers import create_testrun_results
...
class AnsibleExecutorPlugin(ExecutorPlugin):
    ...
    def validate(self):
        ...
        schema_validator(
            schema_data=self.build_profile(self.execute),
        ...
        for index, shell in enumerate(self.shell):
            ...
            if ignorerc:
                ...
                if result["rc"] not in validrc:
                    self.status = 1
                    self.logger.error(
                        "Command %s failed. Host=%s rc=%d\nError: Please look at the"
                        " scenario log for failure."
                        % (shell["command"], result["host"], result["rc"])
                    )
                    self.logger.debug(
                        "During the failure these messages were thrown as a part of"
                        " stderr:\n %s" % result["err"]
                    )
            ...
            if self.status == 1:
                ...
                self.logger.info(
                    "Successfully executed command : %s" % shell["command"]
                )
    ...
    def __playbook__(self):
        ...
        for index, playbook in enumerate(self.playbook):
            ...
            if ignorerc:
                self.logger.info("Ignoring the rc for: %s" % playbook["name"])
            elif results[0] != 0:
                self.status = 1
                if results[1]:
                    self.logger.debug(
                ...
                raise ArchiveArtifactsError(
                    "Playbook %s failed to run.\nPlease look at the scenario"
                    " log for ansible failure." % playbook["name"]
                )
            else:
                self.logger.info(
                    "Successfully executed playbook : %s" % playbook["name"]
                )
    ...
    def __artifacts__(self):
        ...
        if results[0] != 0:
            ...
            raise TefloExecuteError(
                "A failure occurred while trying to copy " "test artifacts."
            )
                ...
                if not self.ans_service.ans_extra_vars["localhost"]:
                    ...
                    art_list = [
                        "/".join(
                            a.replace("’", "")
                            .split("->")[-1]
                            .split("/")[(len(dest_path_parts) - 1) :]
                        )
                        for a in temp_list
                    ]
                self.logger.info(
                    "Copied the artifact(s), %s, from %s" % (art_list, r["host"])
                )
        ...
        if (
            self.config.get("RUNNER_TESTRUN_RESULTS")
            and self.config.get("RUNNER_TESTRUN_RESULTS").lower() == "false"
        ):
            self.execute.testrun_results = {}
        else:
            self.execute.testrun_results = create_testrun_results(
                self.injector.inject_list(self.execute.artifact_locations), self.config
            )
    ...
    def _print_testrun_results(self):
        ...
        if self.execute.testrun_results and self.execute.testrun_results.get(
            "individual_results"
        ):
            self.logger.info("\n")
            self.logger.info("-" * 79)
            ...
            self.logger.info(
                " * Total Tests             : %s"
                % self.execute.testrun_results["aggregate_testrun_results"][
                    "total_tests"
                ]
            )
            self.logger.info(
            ...
            if len(self.execute.testrun_results.get("individual_results")) > 1:
                ...
                for res in self.execute.testrun_results.get("individual_results"):
                    for keys, values in res.items():
                        ...
                        self.logger.info(
                            " * Total Tests             : %s" % values["total_tests"]
                        )
                        self.logger.info(
                            " * Failed Tests            : %s" % values["failed_tests"]
                        )
                        self.logger.info(
                            " * Error Tests             : %s" % values["error_tests"]
                        )
                        self.logger.info(
                            " * Skipped Tests           : %s" % values["skipped_tests"]
                        )
                        self.logger.info(
                            " * Passed Tests            : %s" % values["passed_tests"]
                        )
                        self.logger.info("-" * 79)
        else:
            self.logger.info(
                " No artifacts were collected OR artifacts collected had no xml files"
            )
            self.logger.info("-" * 79 + "\n")
