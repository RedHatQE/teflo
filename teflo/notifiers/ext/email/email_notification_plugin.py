...
from ....helpers import generate_default_template_vars
...
class EmailNotificationPlugin(NotificationPlugin):
    ...
    __schema_file_path__ = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "schema/schema.yml")
    )
    __schema_exts_path__ = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "schema/email_extensions.py")
    )
    ...
    def __init__(self, notification):
        ...
        self.subject = getattr(self.notification, "subject", "Teflo Notification.")
        ...
        self.attachments = [
            os.path.abspath(a) for a in getattr(self.notification, "attachments", [])
        ]
    ...
    def send_message(self):
        ...
        try:
            ...
            if (
                self.creds_params.get("smtp_starttls", False)
                and self.creds_params.get("smtp_starttls") == "True"
            ):
            ...
            if self.creds_params.get("smtp_user", False) and self.creds_params.get(
                "smtp_password", False
            ):
                if smtp.has_extn("AUTH"):
                    try:
                        smtp.login(
                            self.creds_params.get("smtp_user"),
                            self.creds_params.get("smtp_password"),
                        )
                    except smtplib.SMTPAuthenticationError:
                        self.logger.error(
                            "Failed to authentication. Please check the username and/or password."
                        )
                        raise
                    except smtplib.SMTPException:
                        self.logger.error(
                            "Looks like no suitable authentication method was found."
                        )
                        raise
                else:
                    raise TefloNotifierError(
                        "Authentication is not available for the server."
                    )
    ...
    def _build_msg(self):
        ...
        if self.attachments:
            for a in self.attachments:
                ...
                part.add_header(
                    "Content-disposition", "attachment", filename=os.path.basename(a)
                )
                mail.attach(part)
    ...
    def notify(self):
        ...
        if not self.body and not self.body_tmpl:
            ...
            self.body = template_render(
                os.path.abspath(
                    os.path.join(
                        getattr(self.notification, "workspace"), self.body_tmpl
                    )
                ),
                var_dict,
            )
        ...
        if self.creds_params:
            ...
            raise TefloNotifierError(
                "No credentials for the Email Notifier was found in the teflo.cfg."
            )

    def validate(self):

        schema_validator(
            schema_data=self.build_profile(self.notification),
            schema_creds=self.creds_params,
            schema_files=[self.__schema_file_path__],
            schema_ext_files=[self.__schema_exts_path__],
        )
