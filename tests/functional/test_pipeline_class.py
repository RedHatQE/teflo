...
from teflo.tasks import CleanupTask
from teflo.tasks import ExecuteTask
from teflo.tasks import NotificationTask
from teflo.tasks import OrchestrateTask
from teflo.tasks import ProvisionTask
from teflo.tasks import ReportTask
from teflo.tasks import ValidateTask
from teflo.utils.pipeline import NotificationPipelineBuilder
from teflo.utils.pipeline import PipelineBuilder
from teflo.utils.pipeline import PipelineFactory
...
class TestPipelineBuilder(object):
    ...
    def test_invalid_task_class_lookup(invalid_pipe_builder):
        ...
        assert (
            "Unable to lookup task %s class." % invalid_pipe_builder.name
            in ex.value.args[0]
        )

    @staticmethod
    def test_build_validate_task_pipeline(
        scenario, basic_scenario_graph_with_provision_only
    ):
    ...
    @staticmethod
    def test_build_provision_task_pipeline(
        scenario, basic_scenario_graph_with_provision_only
    ):
    ...
    @staticmethod
    def test_build_orchestrate_task_pipeline(
        scenario, basic_scenario_graph_with_provision_only
    ):
    ...
    @staticmethod
    def test_build_execute_task_pipeline(
        scenario, basic_scenario_graph_with_provision_only
    ):
    ...
    @staticmethod
    def test_build_report_task_pipeline(
        scenario, basic_scenario_graph_with_provision_only
    ):
    ...
    @staticmethod
    def test_build_cleanup_task_pipeline(
        scenario, basic_scenario_graph_with_provision_only
    ):
    ...
    @staticmethod
    def test_build_orchestrate_task_pipeline_with_labels(
        scenario_labels, scenario_graph1
    ):
        """This is to test that orch task with  label will run on only provisioned hosts which match that label"""
        builder = PipelineBuilder(name="orchestrate")
        pipeline = builder.build(
            scenario_labels,
            teflo_options={"labels": ["label3"]},
            scenario_graph=scenario_graph1,
        )
        assert getattr(pipeline, "name") == "orchestrate"
        assert getattr(pipeline, "type") is OrchestrateTask
    ...
    @staticmethod
    def test_build_execute_task_pipeline_with_labels(
        scenario_labels, scenario_graph1, execute3
    ):
...
class TestNotificationPipelineBuilder(object):
    ...
    def test_build_on_start_notify_task_pipeline(scenario):
        ...
        builder = NotificationPipelineBuilder(
            trigger="on_start", current_task="provision"
        )
    ...
    def test_build_on_complete_mixed_success_notify_task_pipeline(scenario):
        ...
        setattr(scenario, "failed_tasks", [])
        builder = NotificationPipelineBuilder(
            trigger="on_complete", current_task="provision"
        )
        pipeline = builder.build(scenario, teflo_options={})
        assert getattr(pipeline, "name") == "notify"
        assert isinstance(getattr(pipeline, "tasks"), list)
    ...
    def test_build_on_complete_on_success_notify_task_pipeline(scenario):
        ...
        setattr(scenario, "failed_tasks", [])
        builder = NotificationPipelineBuilder(
            trigger="on_complete", current_task="orchestrate"
        )
        pipeline = builder.build(scenario, teflo_options={})
        assert getattr(pipeline, "name") == "notify"
        assert isinstance(getattr(pipeline, "tasks"), list)
    ...
    def test_build_on_demand_notify_task_pipeline(scenario):
        ...
        setattr(scenario, "failed_tasks", [])
        builder = NotificationPipelineBuilder(
            trigger="on_demand", current_task="orchestrate"
        )
        pipeline = builder.build(scenario, teflo_options={})
        assert getattr(pipeline, "name") == "notify"
        assert isinstance(getattr(pipeline, "tasks"), list)
