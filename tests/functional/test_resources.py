...
from teflo.orchestrators.ext.ansible_orchestrator_plugin import (
    AnsibleOrchestratorPlugin,
)
...
def host2(default_host_params, config):
    host2 = Asset(
        name="host_count", config=config, parameters=copy.deepcopy(default_host_params)
    )
    return host2
...
def report1(mock_plugin_class, mock_plugin_list, default_report_params, config):
    ...
    return Report(
        name="SampleTest.xml",
        parameters=copy.deepcopy(default_report_params),
        config=config,
    )
...
class TestActionResource(object):
    ...
    def test_create_action_with_name():
        params = dict(description="description goes here.", hosts=["host01"])
    ...
    def test_create_action_with_name_in_parameters():
        params = dict(
            name="action", description="description goes here.", hosts=["host01"]
        )
    ...
    def test_setting_of_resource_id():
        ...
        assert (
            "You cannot set resource_id after class is instantiated." in ex.value.args
        )

    @staticmethod
    def test_create_action_without_hosts():
        params = dict(name="action", description="description goes here.", hosts=None)
        with pytest.raises(TefloActionError) as ex:
            Action(parameters=params)
        assert (
            "Unable to associate hosts to action: action.No hosts "
    ...
    def test_create_action_with_hosts_as_str():
        params = dict(
            description="description goes here.", hosts="host01, host02", key="value"
        )
        action = Action(name="action", parameters=params)
    ...
    def test_create_action_with_invalid_orchestrator():
        params = dict(
            description="description goes here.", hosts=["host01"], orchestrator="abc"
        )
        with pytest.raises(TefloActionError) as ex:
...
class TestReportResource(object):
    ...
    @mock.patch("teflo.resources.reports.get_importers_plugin_list")
    @mock.patch("teflo.resources.reports.get_importer_plugin_class")
    def test_timeout_from_scenario(
        mock_plugin_class, mock_plugin_list, timeout_param_report, config
    ):
        mock_plugin_list.return_value = ["polarion"]
        mock_plugin_class.return_value = ImporterPlugin
    ...
    @mock.patch("teflo.resources.reports.get_importers_plugin_list")
    @mock.patch("teflo.resources.reports.get_importer_plugin_class")
    def test_timeout_from_cfg(
        mock_plugin_class, mock_plugin_list, config, report_params_np
    ):
        mock_plugin_list.return_value = ["polarion"]
        mock_plugin_class.return_value = ImporterPlugin
    ...
    def test_create_report_without_name():
        with pytest.raises(TefloReportError) as ex:
            Report()
        assert "Unable to build report object. Name field missing!" in ex.value.args[0]
    ...
    def test_create_report_without_executes(
        mock_plugin_class, mock_plugin_list, default_report_params, config
    ):
        params = copy.deepcopy(default_report_params)
        del params["executes"]
        mock_plugin_list.return_value = ["polarion"]
    ...
    @mock.patch("teflo.resources.reports.get_importers_plugin_list")
    @mock.patch("teflo.resources.reports.get_importer_plugin_class")
    def test_create_report_with_executes_as_str(
        mock_plugin_class, mock_plugin_list, default_report_params, config
    ):
        ...
        mock_plugin_class.return_value = ImporterPlugin
        report = Report(
            name="test.xml", parameters=default_report_params, config=config
        )
        assert isinstance(report.executes, list)
    ...
    @mock.patch("teflo.resources.reports.get_importers_plugin_list")
    @mock.patch("teflo.resources.reports.get_importer_plugin_class")
    def test_create_report_with_executes_as_str_1(
        mock_plugin_class, mock_plugin_list, default_report_params, config
    ):
    ...
    @mock.patch("teflo.resources.reports.get_importers_plugin_list")
    @mock.patch("teflo.resources.reports.get_importer_plugin_class")
    def test_create_report_without_provider_key(
        mock_plugin_class, mock_plugin_list, report_params_np, config
    ):
...
class TestExecuteResource(object):
    ...
    def test_create_execute_without_name():
        with pytest.raises(TefloExecuteError) as ex:
            Execute()
        assert "Unable to build execute object. Name field missing!" in ex.value.args[0]
    ...
    def test_create_execute_without_hosts():
        ...
        with pytest.raises(TefloExecuteError) as ex:
            Execute(name="execute", parameters=params)
        assert (
            "Unable to associate hosts to executor:execute. No hosts "
            "defined!" in ex.value.args[0]
        )
...
class TestScenarioResource(object):
    ...
    @staticmethod
    def test_children_size_property(
        basic_scenario_graph_with_provision_only: ScenarioGraph,
    ):
    ...
    @staticmethod
    def test_reload_method_assets_count_from_middle_tasks(
        task_list_host_with_3, scenario_res3
    ):
    ...
    @staticmethod
    def test_reload_method_assets_count_reversed_from_middle_tasks(
        task_list_host_with_3, scenario_res3
    ):
    ...
    @staticmethod
    def test_reload_method_assets_tasks_using_labels_and_asset_count(
        task_list_host_1, scenario_res3
    ):
        """This is to test if assets which are not picked up for a task still get added to the scenario to populate
        the results.yml"""
    ...
    @staticmethod
    def test_reload_method_reports_tasks_using_labels(
        task_list_report_2, scenario_res3
    ):
        """This is to test if assets which are not picked up for a task still get added to the scenario to populate
        the results.yml"""
...
class TestAssetResource(object):
    ...
    def test_create_host_invalid_provisioner(self, default_host_params, config):
        params = self.__get_params_copy__(default_host_params)
        params["provisioner"] = "null"
        with pytest.raises(TefloResourceError):
            Asset(name="host01", parameters=params, config=config)

    def test_create_host_with_provisioner_set(self, default_host_params, config):
        params = self.__get_params_copy__(default_host_params)
    ...
    def test_metadata_setter(self, host):
        with pytest.raises(AttributeError) as ex:
            host.metadata = {"k": "v"}
        assert (
            "You cannot set metadata directly. Use function "
    ...
    def test_ansible_params_setter(self, host):
        ...
        assert (
            "You cannot set the ansible parameters directly. This is set"
    ...
    def test_provider_setter(self, host):
        with pytest.raises(AttributeError) as ex:
            host.provider = "null"
        assert (
            "You cannot set the asset provider after asset class is "
    ...
    def test_group_setter(self, host):
        with pytest.raises(AttributeError) as ex:
            host.groups = "test"
        assert (
            "You cannot set the groups after asset class is instantiated."
            in ex.value.args[0]
        )
    ...
    def test_provisioner_setter(self, default_host_params, feature_toggle_config):
        ...
        assert (
            "You cannot set the asset provisioner plugin after asset class "
    ...
    def test_same_groups_name_host_name_error(default_host_params, config):
        ...
        assert (
            "Asset name client cannot be same as groups name ['client']"
            in ex.value.args[0]
        )
...
class TestNotificationResource(object):
    ...
    def test_create_notification_with_name():
        params = dict(description="description goes here.", notifier="email-notifier")
    ...
    def test_create_notification_with_name_in_parameters():
        params = dict(
            name="note", description="description goes here.", notifier="email-notifier"
        )
    ...
    def test_create_notification_without_notifier():
        params = dict(name="note", description="description goes here.")
        note = Notification(parameters=params)
    ...
    def test_create_notification_unregistered_notifier():
        params = dict(
            name="note", description="description goes here.", notifier="blah"
        )
        with pytest.raises(TefloResourceError) as ex:
            Notification(parameters=params)
    ...
    def test_notification_load_params():
        params = dict(
            description="description goes here.", notifier="email-notifier", foo="bar"
        )
        note = Notification(name="note", parameters=params)
        assert note.foo
