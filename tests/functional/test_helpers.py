...
from teflo.helpers import check_for_var_file
from teflo.helpers import create_aggregate_testrun_results
from teflo.helpers import create_individual_testrun_results
from teflo.helpers import DataInjector
from teflo.helpers import filter_notifications_to_skip
from teflo.helpers import filter_resources_labels
from teflo.helpers import find_artifacts_on_disk
from teflo.helpers import get_ans_verbosity
from teflo.helpers import get_default_provisioner_plugin
from teflo.helpers import mask_credentials_password
from teflo.helpers import schema_validator
from teflo.helpers import set_task_class_concurrency
from teflo.helpers import sort_tasklist
...
def task_host(task_concurrency_config):
    params = dict(groups="test_group", ip_address="1.2.3.4")
    host = Asset(name="Host01", config=task_concurrency_config, parameters=params)
    return host
...
def task_report(mock_plugin_class, mock_plugin_list, task_concurrency_config):
    ...
    report = Report(
        name="/tmp/dummy.xml", parameters=params, config=task_concurrency_config
    )
...
def data_folder():
    ...
    os.system(
        "mkdir /tmp/1 /tmp/.results /tmp/.results/artifacts /tmp/.results/logs "
    ...
    os.system(
        "touch /tmp/.results/artifacts/localhosts/payload_eg/results/junit_example.xml "
...
class TestDataInjector(object):
    ...
    def test_inject_dictionary_04(self, data_injector):
        cmd = data_injector.inject_dictionary(
            dict(ans_args="-c -e { node01.random } { node01.random_01[0]}")
        )
    ...
    def test_inject_list_03(self, data_injector):
        cmd = data_injector.inject_list(
            ["hello", ["world", "{ node01.metadata.k5[0].k1 }"]]
        )
        assert cmd[0] == "hello"
...
def test_validate_render_scenario_incorrect_iterate_method(task_concurrency_config):
    ...
    with pytest.raises(ValueError) as ex:
        scenario_graph = validate_render_scenario(
            "../assets/correct_include_descriptor.yml", task_concurrency_config
        )
    assert (
        "the iterate method value set in teflo.cfg is incorrect wrong_val"
        in ex.value.args[0]
    )
...
def test_validate_render_scenario_no_include(task_concurrency_config):
    scenario_graph = validate_render_scenario(
        os.path.abspath("../assets/no_include.yml"), task_concurrency_config
    )
...
def test_validate_render_scenario_wrong_include(task_concurrency_config):
    with pytest.raises(TefloError) as ex:
        validate_render_scenario(
            "../assets/wrong_include_descriptor.yml", task_concurrency_config
        )
    assert (
...
def test_set_task_concurrency_provision_is_false(host):
    for task in host.get_tasks():
        if task["task"].__task_name__ == "provision":
            assert (
                set_task_class_concurrency(task, host)["task"].__concurrent__ is False
            )
...
def test_set_task_concurrency_report_is_false(report_resource):
    for task in report_resource.get_tasks():
        if task["task"].__task_name__ == "report":
            assert (
                set_task_class_concurrency(task, report_resource)["task"].__concurrent__
                is False
            )
...
def test_set_task_concurrency_provision_is_true(task_host):
    for task in task_host.get_tasks():
        if task["task"].__task_name__ == "provision":
            assert (
                set_task_class_concurrency(task, task_host)["task"].__concurrent__
                is True
            )
...
def test_set_task_concurrency_report_is_true(task_report):
    for task in task_report.get_tasks():
        if task["task"].__task_name__ == "report":
            assert (
                set_task_class_concurrency(task, task_report)["task"].__concurrent__
                is True
            )
...
def test_find_artifacts_on_disk_art_location_1(data_folder):
    ...
    """
    art_location = [
        "artifacts/localhosts/payload_eg/",
        "artifacts/localhosts/payload_eg/results",
...
def test_create_individual_testrun_results_with_wrong_xml(mock_method):
    ...
    with pytest.raises(TefloError) as ex:
        ...
        assert (
            "The xml file ../assets/artifacts/host01/sample1.xml is malformed"
            in ex.value.args
        )
...
def test_filter_notifications_to_skip_01(
    teflo1, notification_on_start_resource, notification_default_resource
):
...
def test_filter_notifications_to_skip_02(
    teflo1, notification_on_start_resource, notification_default_resource
):
