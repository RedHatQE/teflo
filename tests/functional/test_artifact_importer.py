...
def execute(artifact_locations):
    e = mock.MagicMock(
        spec=Execute, name="pytest", artifact_locations=artifact_locations
    )
...
@pytest.fixture(scope="class")
def plugin():
    pg = mock.MagicMock(spec=ImporterPlugin, __plugin_name__="polarion")
...
@pytest.fixture(scope="class")
def report(
    default_report_params, plugin, report_config, default_profile_params, execute
):
    report = mock.MagicMock(spec=Report, name="report", config=report_config)
...
class TestArtifactImporter(object):
    ...
    def test_artifact_importer_validate_no_artifact_location(
        mock_find_disks, artifact_importer, plugin, artifact_locations, execute
    ):
        ...
        mock_find_disks.return_value = [os.path.join("/tmp", dir) for dir in locations]
    ...
    @staticmethod
    @mock.patch("teflo.importers.artifact_importer.find_artifacts_on_disk")
    def test_artifact_importer_validate_artifact_locations_artifact_on_disk(
        mock_find_disks, artifact_importer, plugin, artifact_locations, execute
    ):
    ...
    @staticmethod
    @mock.patch("teflo.importers.artifact_importer.find_artifacts_on_disk")
    def test_artifact_importer_validate_no_execute_artifacts_on_disk(
        mock_find_disks, plugin, report, artifact_locations
    ):
    ...
    @staticmethod
    @mock.patch("teflo.importers.artifact_importer.find_artifacts_on_disk")
    def test_artifact_importer_validate_no_artifact_on_disk(
        mock_find_disks, artifact_importer, plugin, execute, report
    ):
    ...
    @staticmethod
    @mock.patch("teflo.importers.artifact_importer.find_artifacts_on_disk")
    def test_artifact_importer_import_artifacts(
        mock_find_disks, artifact_importer, plugin, artifact_locations, execute
    ):
    ...
    @staticmethod
    @mock.patch("teflo.importers.artifact_importer.find_artifacts_on_disk")
    def test_artifact_importer_import_artifacts_failure(
        mock_find_disks, artifact_importer, plugin, artifact_locations
    ):
