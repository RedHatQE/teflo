...
def resource_checker1(config, scenario):
    ...
    res_check.ans_service = AnsibleService(
        config, hosts=[], all_hosts=[], ansible_options=None
    )
    return res_check
...
class TestResourceChecker(object):
    ...
    def test_check_custom_resource_script(resource_checker1):
        ...
        assert (
            "Failed to run resource_check validation for playbook/script"
            in ex.value.args[0]
        )
    ...
    def test_check_custom_resource_service1(resource_checker4):
        ...
        assert (
            not "will not be run! Not all external resources are available or valid"
            in ex.value.args[0]
        )
